...
from association.result_delivery import (
    DATABASE_URI, DATABASE, RULES_COLLECTION, ITEM_SETS_COLLECTION
)
...
def result_delivery(run_date: str) -> None:
    ...
    path_suffix = f"run_date={run_date}"
    rules_df = delta_io.read(path=RULES_PATH).select(
       "store_id", "antecedent", "consequent", "confidence", "lift", "support"
    )

    item_sets_df = delta_io.read(
        path=ITEM_SETS_PATH, path_suffix=path_suffix
    ).select("store_id", "items", "freq")
    ...
    rules_df = rules_df.drop("product_description_en")
